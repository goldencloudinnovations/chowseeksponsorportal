...
const webhookSecret = Deno.env.get('RESTAURANT_STRIPE_WEBHOOK_SECRET');
if (!stripeKey || !webhookSecret) throw new Error('Restaurant Stripe webhook secrets are required.');
const stripe = new Stripe(stripeKey, { apiVersion: '2026-07-29.dahlia' });
const cryptoProvider = Stripe.createSubtleCryptoProvider();
const starterPrice = Deno.env.get('STRIPE_RESTAURANT_STARTER_PRICE_ID');
const growthPrice = Deno.env.get('STRIPE_RESTAURANT_GROWTH_PRICE_ID');
...
async function syncSubscription(admin: SupabaseClient, subscription: Stripe.Subscription) {
  ...
  if (subscription.metadata.chowseek_product !== 'restaurant_portal' && !restaurantId) return;
  const patch = {
    ...
    try {
      ...
      switch (event.type) {
        ...
        case 'checkout.session.completed': {
          ...
          if (session.metadata?.chowseek_product !== 'restaurant_portal') break;
