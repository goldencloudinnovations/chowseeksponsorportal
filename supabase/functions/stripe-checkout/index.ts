...
function integrationIdentifier() {
  ...
  return `chowseek_portal_${suffix}`;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!restaurantId || !priceId) throw new Error('Unknown plan. Configure the matching Stripe Price ID secret.');
    ...
    if (!customerId) {
      const customer = await stripe.customers.create({ name: restaurant.name, email: user.email ?? undefined, metadata: { restaurant_id: restaurantId } });
      customerId = customer.id;
      const { error: saveError } = await admin.from('restaurants').update({ stripe_customer_id: customerId, updated_at: new Date().toISOString() }).eq('id', restaurantId);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { restaurant_id: restaurantId, plan_key: planKey },
      subscription_data: { metadata: { restaurant_id: restaurantId, plan_key: planKey } },
