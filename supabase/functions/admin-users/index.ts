...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'list') {
      ...
      if (authError) throw authError; if (membershipError) throw membershipError;
    ...
    if (action === 'invite') {
      ...
      const { error: memberError } = await admin.from('restaurant_memberships').upsert({ restaurant_id: restaurantId, user_id: data.user.id, role, active: true, updated_at: new Date().toISOString() }, { onConflict: 'restaurant_id,user_id' });
