...
-- Run after restaurant_single_cpm_billing.sql.

alter table chowseek_private.restaurant_billing_config
  add column if not exists stripe_portal_configuration_id text;

-- The first billing SQL may already have created the 4-argument setter. Drop the
-- old RPC signatures before expanding them so PostgREST never has ambiguous overloads.
...
-- Defense in depth: these private tables are server-only even though PostgreSQL's
-- default new-table grants are already restrictive.
