...
function canEditRestaurant() { if (isAdmin)
    return true; const m = currentMembership(); return m?.role === 'owner' || m?.role === 'editor'; }
function canManageBilling() { if (isAdmin)
    return true; return currentMembership()?.role === 'owner'; }
...
function slugify(value) { const base = value.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 48) || 'restaurant'; return `${base}-${crypto.randomUUID().slice(0, 6)}`; }
function placementStatus(p) { const now = Date.now(); if (!p.active)
    return ['Paused', '']; if (!billingActive())
...
function showPassword(mode) {
    ...
    $('current-password').required = !recovery;
...
async function loadAccess() {
    ...
    if (!isAdmin && !memberships.length) {
        showOnly('denied');
        return;
    }
    let restaurantQuery = supabase.from('restaurants').select('*').order('name');
    if (!isAdmin)
        restaurantQuery = restaurantQuery.eq('id', memberships[0].restaurant_id);
    const { data, error: restaurantsError } = await restaurantQuery;
    if (restaurantsError)
        throw restaurantsError;
    restaurants = (data ?? []);
    if (!isAdmin && !restaurants.length) {
        showOnly('denied');
        return;
    }
...
function renderShell() {
    ...
    const userRestaurant = $('create-user-restaurant');
    userRestaurant.innerHTML = restaurants.map(x => `<option value="${esc(x.id)}">${esc(x.name)}</option>`).join('');
    $('create-user-submit').disabled = !restaurants.length;
...
function renderBilling() {
    ...
    if (!r)
        return;
    $('billing-status').textContent = r.subscription_status ?? 'not subscribed';
    $('billing-plan').textContent = '$20 CPM';
    $('billing-renewal').textContent = r.current_period_end ? new Date(r.current_period_end).toLocaleDateString() : '—';
    const manage = $('manage-billing');
    const start = $('start-billing');
    ...
    start.disabled = !canManageBilling() || managed;
    start.textContent = managed ? 'Billing active' : 'Start sponsorship billing';
...
async function loadAdminUsers() {
    if (!isAdmin)
        return;
    const data = await invoke('restaurant-admin-users', { action: 'list' });
...
function renderAdminUsers() {
    ...
    $('admin-users').innerHTML = adminUsers.map(u => {
        ...
        const restaurant = m ? restaurants.find(r => r.id === m.restaurant_id) : null;
...
$('login-form').addEventListener('submit', async (event) => { event.preventDefault(); notice(); const email = $('login-email').value.trim(); const password = $('login-password').value; const { error } = await supabase.auth.signInWithPassword({ email, password }); if (error)
    return notice(error.message, true); recoveringPassword = false; await loadAccess().catch((e) => notice(e.message, true)); });
...
$('password-form').addEventListener('submit', async (e) => { e.preventDefault(); notice(); const next = $('new-password').value; const confirm = $('confirm-password').value; if (next.length < 12)
    return notice('Use at least 12 characters.', true); if (next !== confirm)
    return notice('New passwords do not match.', true); if (passwordMode !== 'recovery') {
    const current = $('current-password').value;
    const { data: { user } } = await supabase.auth.getUser();
    if (!user?.email)
        return notice('Your session is missing an email address.', true);
    const { error: verifyError } = await supabase.auth.signInWithPassword({ email: user.email, password: current });
    if (verifyError)
        return notice('Current password is incorrect.', true);
} const { error } = await supabase.auth.updateUser({ password: next }); if (error)
    return notice(error.message, true); recoveringPassword = false; history.replaceState({}, '', location.pathname); await supabase.auth.signOut(); showOnly('login'); notice('Password changed. Sign in with your new password.'); });
$('cancel-password').addEventListener('click', async () => { if (passwordMode === 'account') {
    showOnly('portal');
...
$('restaurant-switcher').addEventListener('change', async (e) => { selectedRestaurantId = e.target.value; resetPlacementForm(); renderShell(); await loadPlacements(); await openTab('restaurant'); });
document.querySelectorAll('.tab').forEach(button => button.addEventListener('click', () => openTab(button.dataset.tab ?? 'restaurant')));
$('restaurant-form').addEventListener('submit', async (e) => { e.preventDefault(); if (!canEditRestaurant())
    return; const payload = { name: $('restaurant-name').value.trim(), address: $('restaurant-address').value.trim() || null, phone: $('restaurant-phone').value.trim() || null, website: $('restaurant-website').value.trim() || null, description: $('restaurant-description').value.trim() || null, latitude: $('restaurant-latitude').value ? Number($('restaurant-latitude').value) : null, longitude: $('restaurant-longitude').value ? Number($('restaurant-longitude').value) : null, updated_at: new Date().toISOString() }; const { error } = await supabase.from('restaurants').update(payload).eq('id', selectedRestaurantId); if (error)
...
$('placements-list').addEventListener('click', async (event) => { const target = event.target; const id = target.dataset.id; if (!id)
...
$('start-billing').addEventListener('click', async () => { const r = currentRestaurant(); if (!r || !canManageBilling())
    return; try {
    const data = await invoke('restaurant-stripe-checkout', { restaurant_id: r.id });
    location.href = data.url;
}
catch (error) {
    notice(error instanceof Error ? error.message : String(error), true);
} });
$('manage-billing').addEventListener('click', async () => { const r = currentRestaurant(); if (!r || !canManageBilling())
    return; try {
    const data = await invoke('restaurant-stripe-portal', { restaurant_id: r.id });
    location.href = data.url;
}
catch (error) {
    notice(error instanceof Error ? error.message : String(error), true);
} });
$('create-restaurant-form').addEventListener('submit', async (e) => { e.preventDefault(); const name = $('new-restaurant-name').value.trim(); if (!name)
    return; const { error } = await supabase.from('restaurants').insert({ name, slug: slugify(name) }); if (error)
    return notice(error.message, true); $('create-restaurant-form').reset(); notice('Restaurant created.'); await loadAccess(); await openTab('admin'); });
$('create-user-form').addEventListener('submit', async (e) => { e.preventDefault(); try {
    const email = $('create-user-email').value.trim();
    const password = $('create-user-password').value;
    const restaurant_id = $('create-user-restaurant').value;
    const role = $('create-user-role').value;
    await invoke('restaurant-admin-users', { action: 'create_user', email, password, restaurant_id, role });
    $('create-user-form').reset();
    notice('Account created. The user must change the temporary password at next sign-in.');
    await loadAdminUsers();
}
catch (error) {
    notice(error instanceof Error ? error.message : String(error), true);
} });
$('refresh-admin').addEventListener('click', async () => { try {
    await loadAdminUsers();
...
$('admin-restaurants').addEventListener('click', async (e) => { const target = e.target; if (!target.classList.contains('open-restaurant'))
    return; const id = target.dataset.id; if (!id)
    return; selectedRestaurantId = id; renderShell(); await loadPlacements(); await openTab('restaurant'); });
$('admin-restaurants').addEventListener('change', async (e) => { const target = e.target; const id = target.dataset.id; if (!id)
    return; try {
    if (target.classList.contains('admin-limit'))
        await invoke('restaurant-admin-users', { action: 'update_restaurant', restaurant_id: id, [target.dataset.field ?? 'max_users']: Number(target.value) });
    if (target.classList.contains('admin-active'))
        await invoke('restaurant-admin-users', { action: 'update_restaurant', restaurant_id: id, active: target.checked });
    notice('Restaurant settings saved.');
    await loadAccess();
    await openTab('admin');
}
catch (error) {
...
$('admin-users').addEventListener('click', async (e) => { const target = e.target; const userId = target.dataset.user; if (!userId)
    return; try {
    if (target.classList.contains('save-role')) {
        const restaurantId = target.dataset.restaurant ?? '';
        const select = document.querySelector(`.user-role[data-user="${CSS.escape(userId)}"]`);
        await invoke('restaurant-admin-users', { action: 'set_access', user_id: userId, restaurant_id: restaurantId, active: true, role: select?.value ?? 'viewer' });
        notice('User role saved.');
    }
    else if (target.classList.contains('toggle-user-access')) {
        await invoke('restaurant-admin-users', { action: 'set_access', user_id: userId, restaurant_id: target.dataset.restaurant ?? '', active: target.dataset.active === 'true' });
        notice('User access updated.');
    }
    else if (target.classList.contains('reset-user-password')) {
        const password = prompt('Temporary password (12+ characters). The user will be required to change it at next sign-in.');
        if (!password)
            return;
        await invoke('restaurant-admin-users', { action: 'reset_password', user_id: userId, password });
        notice('Temporary password set. Password change will be required at next sign-in.');
    }
    else if (target.classList.contains('delete-user')) {
        if (!confirm('Delete this restaurant portal account?'))
            return;
        await invoke('restaurant-admin-users', { action: 'delete', user_id: userId });
...
supabase.auth.onAuthStateChange((event) => { if (event === 'PASSWORD_RECOVERY') {
    ...
    queueMicrotask(() => showPassword('recovery'));
} if (event === 'SIGNED_OUT') {
    currentUser = null;
    queueMicrotask(() => showOnly('login'));
} });
resetPlacementForm();
(async () => { const { data: { session } } = await supabase.auth.getSession(); if (session) {
    await loadAccess().catch((e) => notice(e.message, true));
}
else
    showOnly('login'); })();
